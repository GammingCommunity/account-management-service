	...
	protected function handleFriendAccount(Account &$lookingAccount, ?AccountRelationship $relasitonship1, ?AccountRelationship $relasitonship2, AccountLookingResult &$accountLookingResult)
	{
		...
		if (
			...
			if ($lookingAccount->setting->birthmonth_privacy === AccountPrivacyType::PRIVATE) {
				if ($lookingAccount->birthMonth->privacy_type === AccountPrivacyType::PRIVATE) {
					$lookingAccount->birthmonth = null;
				}
			}
			if ($lookingAccount->birthYear) {
				if (
					$lookingAccount->birthYear->privacy_type === AccountPrivacyType::PRIVATE
				) {
			...
			if ($lookingAccount->phone) {
				if (
					$lookingAccount->phone->privacy_type === AccountPrivacyType::PRIVATE
				) {
			...
			if ($lookingAccount->email) {
				if (
					$lookingAccount->email->privacy_type === AccountPrivacyType::PRIVATE
				) {
	...
	protected function handleStrangerAccount(Account &$lookingAccount, ?AccountRelationship $relasitonship1, ?AccountRelationship $relasitonship2, AccountLookingResult &$accountLookingResult)
	{
		...
		if (!$relasitonship1 && !$relasitonship2) {
			...
			if ($lookingAccount->birthMonth) {
				if (
					$lookingAccount->birthMonth->privacy_type !== AccountPrivacyType::PUBLIC
				) {
			...
			if ($lookingAccount->birthYear) {
				if (
					$lookingAccount->birthYear->privacy_type !== AccountPrivacyType::PUBLIC
				) {
					$lookingAccount->account_info_birth_year_id = null;
				}
			}
			if ($lookingAccount->phone) {
				if (
					$lookingAccount->phone->privacy_type !== AccountPrivacyType::PUBLIC
				) {
					$lookingAccount->account_info_phone_id = null;
				}
			}
			if ($lookingAccount->email) {
				if (
					$lookingAccount->email->privacy_type !== AccountPrivacyType::PUBLIC
				) {
					$lookingAccount->account_info_email_id = null;
				}
			}
