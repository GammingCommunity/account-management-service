...
class SearchAccounts
{
	...
	public function __invoke($rootValue, array $args, GraphQLContext $context, ResolveInfo $resolveInfo): array
	{
		...
		exit(get_class($this->findAccountsByString($searchKey)));
		
		$currentAccount = $rootValue['verified_account'];

		if ($currentAccount) {
			$lookingAccounts = Account::find($searchKey);
	...
	protected function findAccounts(string $key): array{
		...
		if(is_numeric($key)){
			if($this->findAccountsByKey($key)){
				array_push($accounts, $this->findAccountsByKey($key));
			}else{
	...
	protected function findAccountsByKey($key){
		return Account::find($key);
	}

	protected function findAccountsByString($key){
		return Account::where('describe', 'like', "%{$key}%")->orWhere('name', 'like', "%{$key}%")->get();
	}
