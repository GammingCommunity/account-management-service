...
class SendFriendRequest
{
	...
	public function __invoke($rootValue, array $args, GraphQLContext $context, ResolveInfo $resolveInfo): bool
	{
		...
		if ($currentAccount) {
			...
			})->where('relationship_type', '<>', AccountRelationshipType::FRIEND_REQUEST)->where('relationship_type', '<>', AccountRelationshipType::STRANGER)->get(['relationship_type', 'sender_account_id', 'receiver_account_id']);
		
			if($this->checkRelationships($relationships)){
	...
	protected function checkRelationships(?Collection $relationships): bool{
		if (!$relationships) {
			return true;
		}
